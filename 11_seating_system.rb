...
#   end .join("\n")
# end
# input = DAT.each_line.map do |line|
#   line.chars.map { |c| c == 'L' ? 0 : 0.01 }.tap { |row| row.unshift(0.01) }.tap { |row| row.push(0.01) }
# end
# input.unshift([0.01] * input.first.length)
# input.push([0.01] * input.first.length)
# @i = input
# i = 0
# until  i > 3
#   temp = @i.map(&:itself)
#   @i = @i.map.with_index do |row, ri|
#     row.map.with_index do |seat, si|
#       surrounding = @i[ri - 1..ri + 1].transpose[si - 1..si + 1].tap { |a| a && a[1] && a[1][1] = 0.01 }
#       if ri.zero? || si.zero? || ri == input.length - 1 || si == input.first.length - 1 || seat == 0.01
#         0.01
#       else
#         surrounding.flatten.sum.to_i >= 4 ? 0 : 1
#       end
#     end
#   end
#   puts map_grid(temp)
